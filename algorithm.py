...
ACCOUNT_BALANCE      = 2500   # update to your actual balance
...
MAX_TOTAL_LOSS_PCT   = 0.10   # 10% max total drawdown
...
def is_trading_session(ts):
    """London open through NY close — 07:00–21:00 UTC."""
    hour = datetime.fromtimestamp(ts, tz=timezone.utc).hour
    return 7 <= hour <= 21


def lot_size(sl_distance, account_balance=None, risk_pct=None):
    """Size position to exactly risk_pct of account balance."""
...
def check_risk_limits(current_balance, daily_start_balance=None, initial_balance=None):
    """
    Returns (allowed, reason) tuple.
    Blocks trading if daily loss > 4% or total drawdown > 10%.
    """
    balance  = current_balance
    initial  = initial_balance  or ACCOUNT_BALANCE
    day_start = daily_start_balance or balance

    daily_loss_pct = (day_start - balance) / day_start if day_start > 0 else 0
    total_loss_pct = (initial - balance)   / initial   if initial  > 0 else 0

    if daily_loss_pct >= MAX_DAILY_LOSS_PCT:
        return False, f"Daily loss limit hit ({daily_loss_pct:.1%} >= {MAX_DAILY_LOSS_PCT:.0%})"
    if total_loss_pct >= MAX_TOTAL_LOSS_PCT:
        return False, f"Total drawdown limit hit ({total_loss_pct:.1%} >= {MAX_TOTAL_LOSS_PCT:.0%})"
    return True, "OK"
...
_DB_PATH = os.environ.get(
    "TRADE_HISTORY_DB",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "trade_history.db")
)
...
def get_adaptive_threshold():
    """Compute confidence threshold from recent trade history. Never mutates globals."""
    conn = _get_db()
    try:
        rows = conn.execute(
...
def compute_confidence(signal, trend, rsi_val, htf_bias):
    ...
    if signal["type"] == "buy" and trend == "bullish" and htf_bias == "bullish":
        ...
        score += 20
    if signal["type"] == "buy":
        if 55 < rsi_val <= 70:    score += 30
        elif 45 <= rsi_val <= 55: score += 15
...
def detect_entry_signals(df, atr_series, htf_bias_map, confidence_threshold=60, for_display=True):
    ...
    for i in range(200, len(df)):
        ...
        if candidates:
            signals.append(max(candidates, key=lambda x: x["confidence"]))
...
def mt5_connect():
    ...
    if mt5.initialize():
        ...
        if info and info.login > 0:
            print(f"[MT5] Connected — {info.server} | Balance: ${info.balance:.2f}")
            return True
        mt5.shutdown()
...
def execute_trade_mt5(signal, symbol="XAUUSD", lot=None):
    ...
    try:
        ...
        # ── Risk limit check using live account balance ────────
        info = mt5.account_info()
        if info:
            ...
            if not allowed:
                print(f"[MT5] Trade BLOCKED — {reason}")
                mt5.shutdown()
                return None
        ...
        if sym_info is None:
            ...
            mt5.shutdown(); return None
        if not sym_info.visible or not sym_info.trade_mode:
        ...
        if signal["type"] == "buy":
            if price - sl < min_dist: sl = round(price - min_dist, digits)
            if tp - price < min_dist: tp = round(price + min_dist * 2, digits)
        else:
            if sl - price < min_dist: sl = round(price + min_dist, digits)
            if price - tp < min_dist: tp = round(price - min_dist * 2, digits)
        ...
        if result and result.retcode == mt5.TRADE_RETCODE_DONE:
            print(f"[MT5] ✓ {signal['type'].upper()} {symbol} @ {price:.5f} | Lot {lot}")
        else:
...
def generate_ai_analysis(df, signals):
    ...
    return (f"Market Analysis:\n- Price: {round(last_price, 5)}\n"
            f"- Trend: {trend}\n- Momentum: {momentum} (RSI {round(rsi_val,1)})\n"
            f"- Volatility (ATR): {round(atr_val, 5)}\n"
            f"Structure: {trend} with {momentum} momentum. Last signal: {last_sig}.\n"
            f"Strategy: Trade with dominant trend. Avoid counter-trend entries "
...
def generate_summary(bias, htf_bias, last_rsi, last_close, ema20, ema50, ema200,
    ...
    if signals:
        ...
        sig_desc = (f"Latest: {s['type'].upper()} @ {s['price']} · "
                    f"SL {s['sl']} · TP {s['tp']} · 1:{s['rr']} · {s.get('confidence','—')}% conf.")
    else:
...
def run_analysis(candles, symbol="EURUSD", timeframe="1h"):
    ...
    # Drop still-forming candle (its OHLC keeps changing until bar closes)
    if len(df) >= 3:
    ...
    # Local threshold — never mutates globals
