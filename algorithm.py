...
# MT5 only available on Windows — safe import
try:
...
def get_htf_bias(df):
    ...
    for dt in h4.index:
        ...
        e20   = h4_ema20.loc[dt]
        e50   = h4_ema50.loc[dt]
        bias  = "bullish" if close > e20 > e50 else \
                "bearish" if close < e20 < e50 else "neutral"
        for h in range(4):
...
def detect_fvg(df):
    """Fair Value Gaps — stores candle index for recency checks."""
    fvgs = []
...
# ══════════════════════════════════════════════════════════════
# ORDER BLOCKS  (no volume filter — forex has no real volume)
# ══════════════════════════════════════════════════════════════
...
def detect_order_blocks(df, lookback=10):
    ...
    for i in range(lookback, len(df) - 1):
        body   = abs(c[i] - o[i])
        thresh = 1.1 * atr.iloc[i]   # slightly looser than 1.2
        if body <= thresh:
            continue
...
# ══════════════════════════════════════════════════════════════
# CONFIDENCE SCORE  (full nuanced scoring)
# ══════════════════════════════════════════════════════════════

def compute_confidence(signal, trend, rsi_val, htf_bias):
    """
    0–100 score across 3 factors:
      40pts — LTF + HTF trend alignment
      30pts — RSI momentum confirmation
      30pts — R:R quality
    """
    score = 0
...
# ══════════════════════════════════════════════════════════════
# MT5 INTEGRATION  (safe — won't crash on Linux/Render)
# ══════════════════════════════════════════════════════════════
...
def generate_ai_analysis(df, signals):
    ...
    momentum   = "strong bullish" if rsi_val > 60 else \
                 "strong bearish" if rsi_val < 40 else "weak / ranging"
...
# ══════════════════════════════════════════════════════════════
# MAIN RUNNER  (full output for frontend)
# ══════════════════════════════════════════════════════════════
